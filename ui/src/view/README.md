...
- `likes.playlist` is a Settings row: Enter edits the name (`Editing::LikedPlaylist`, `Session::set_liked_playlist`, live, saved as `liked_playlist` in `state.toml` when it differs from the config value). A like on a track none of whose sources has a liked-songs list goes into the local playlist of that name (an existing one is reused, else created on first use) and unliking removes it; changing the name leaves the old playlist as an ordinary one.
...
- `TrackList` owns its kind, `ListState`, where a Playlists window is (`Open`: top level, a local
  ...
  toggles the selected track's membership as in any list; a removal asks first (`Session::removal_prompt`), and from the list showing that playlist unfiltered it removes only the cursor row (`TrackList::open_row`, a position; `PendingRows` dims just that row), elsewhere every occurrence. `/` (`Editing::Filter`) always fuzzy-filters the focused window's own list — Playlists' top level by playlist/album name, a Search window's top level by track and collection name as a second layer over its results, and any other list by track — live as typed; Enter locks it in place, still filtered and browsable/actionable normally. A filter stays with its window
  across tab switches and closing; only Esc, opening the `:`-command line, or `reset_for_new_list` clears it.

## Search windows

Every Search window (the `search` tab, the `results` companion) is independent: its query, results, kind filter, opened collection, cursor and `/`-filter are its own. The window holds `TrackList::search`, the id `Session::search(text, replaces)` returned for its last query, and reads that id's `ResultSet` (`core::view_cache`: query, resolved tracks, collections, a generation) through `Session::results(id)`; nothing keeps a "current" search. A new search from the window replaces (forgets) its previous id, and `close_window`/a placement change that hides it (`drop_search`) forgets it too; `Session::forget_search` cancels the source threads (`Search::forget` removes the id from the live set every sink consults) and drops the set, and a hit for a forgotten id is dropped in `on_event`. Blank text only forgets. `Editing::Search(WindowId)` is that window's query input: the shell writes the buffer through to `TrackList::set_input` on every keystroke, and the window draws it in its own title row as `search: <text>█` after the kind bar (`typed_title`: the end stays in view via `tail_fit`, and a red `  (esc to exit)` only while everything fits; the corner `input_line` never shows it). A tab digit (`Action::Tab`), `Action::ToggleWindow`/`Action::ShowWindow` (`:window search`), and `/` (`FocusSearch`) on a Search window with no results yet (`TrackList::awaiting_search`) open the input (`edit_search`); once results are showing, `/` opens the local `/`-filter over them instead, same as any other window. `:search <query>`, placements and `:panes` layouts show the window without editing; commit runs `start_search` on that window, which lists the new results from its top (`set_search`) with the kind filter kept. A `/` typed as the first character is dropped, a digit there switches tabs as before. `search_window` picks where a search goes when no Search list is focused (`:search <query>`): a shown Search window, else the tab.

## Kind bar

`kind_bar.rs` draws and hit-tests the title row's kind bar. The bar sits left, from the tags column, and titles are right-aligned; a static `[f] filter` label (the `CycleKindFilter` key) follows the bar when it fits beside the title without squeezing it. A list has one (`TrackList::has_kind_bar`) at the top level of a Search window (All, Songs, Albums, Playlists) or a Playlists window (All, Albums, Playlists); `is_results` is narrower: a Search top level, whose list mixes tracks and collections. `f` (`cycle_kinds`) or a click on a segment sets `kinds`, bumping `view_gen` with no refetch; if `select` names a row the filter hides, `relayout` resets the filter to All. Album rows never take a hotkey.
...
The setup dialog (`setup.rs`, `SetupModal`) is a floating box over the main view that runs one plugin's setup as a chat: each `Plugin::setup_prompt` (text, `secret`, `default`) is logged, then the answer (masked when secret), then the progress lines the running `setup` sends through its `SetupLog` (`CoreEvent::SetupLine`) and, for that plugin's context, background warnings. The log is a `LogPane` over its own `LogBuf`, scrolled like the Log window; the input line sits under it and takes every key, so a long paste is plain characters, never a command. It is opened from a warnings row or the Settings `set up <source>` row (`open_setup`). `Plugin::setup_answer` checks each answer (an `Err` is logged and the question repeats); Ctrl-U clears the input line, a secret answer is kept exactly as typed. `setup` runs on a spawned thread (one per plugin, `Session::setup_running`; opening the dialog again while one runs only flashes) and reports `SetupDone` with the plugin's own `probe()` after it: `Ok` closes the dialog, anything else keeps it open with the reason (Enter starts over). Esc, the `[ Close ]` button or dropping the modal cancels the run (`SetupLog::cancelled`, which a plugin polls in its waits); an abandoned run still rewires and re-probes its plugin, shows nothing, and a thread stuck in a blocking call finishes unseen.
